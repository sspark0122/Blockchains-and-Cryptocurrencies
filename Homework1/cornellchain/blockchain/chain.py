...
class Blockchain(persistent.Persistent):
    ...
    def get_chain_ending_with(self, block_hash):
        ...
        # Placeholder for (1a)
        chains = []
        # If the provided hash is not found in the chain, return empty list.
        if block_hash not in self.blocks:
            return chains

        # Traverse back to genesis block (first block in the chain).
        chains.append(block_hash)
        while self.blocks[block_hash].is_genesis == False:
